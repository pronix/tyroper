module Wice
  class GridRenderer
    ...
    def column(opts = {}, &block)
      ...
      vc.helper_style   = options[:helper_style]
      vc.table_alias    = options[:table_alias]
      vc.custom_order    = options[:custom_order]
    ...
    def get_row_attributes(ar_object) #:nodoc:
      if @row_attributes_handler
        ...
        unless row_attributes.is_a?(Hash)
          raise WiceGridArgumentError.new("WiceGrid row_attributes block must return a hash containing HTML attributes. The returned value is #{row_attributes.inspect}")
        end
        row_attributes
      else
        {}

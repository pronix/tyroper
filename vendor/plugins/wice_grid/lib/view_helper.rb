module Wice
  module GridViewHelper
    ...
    def scaffolded_grid(grid_obj, opts = {})
    ...
    def call_block_as_erb_or_ruby(grid, block, ar)  #:nodoc:
      if grid.erb_mode?
        capture(ar, &block)
      else
        block.call(ar)
    ...
    def grid_html(grid, table_html_attrs, header_tr_html_attrs, options, rendering) #:nodoc:
      ...
      content = javascript_tag( %$ if (typeof(WiceGridProcessor) == "undefined"){
        ...
        if ((typeof(WiceGridProcessor._version) == "undefined") || ( WiceGridProcessor._version != "0.3")) {
      ...
      grid.each do |ar| # rows
        ...
        before_row_output = if rendering.before_row_handler
          call_block_as_erb_or_ruby(grid, rendering.before_row_handler, ar)           
        else
          nil
        end

        after_row_output = if rendering.after_row_handler
          call_block_as_erb_or_ruby(grid, rendering.after_row_handler, ar)
        else
          nil
        end
        ...
        rendering.each_column(:in_html) do |column|
          ...
          column_block_output = call_block_as_erb_or_ruby(grid, cell_block, ar)
      ...
      content += javascript_tag(
        "#{grid.name} = new WiceGridProcessor('#{grid.name}', '#{base_link_for_filter}', '#{base_link_for_show_all_records}', '#{link_for_export}', '#{parameter_name_for_query_loading}');\n" +
